...
(() => {
  ...
  ui.el.formulario.addEventListener('submit', async (e) => {
    ...
    if (!nomeOk(dados.nome)) {
      ui.marcarErroCampo(ui.el.nome, 'Por favor, use um nome apropriado (sem ofensas).');
      ui.exibirStatus('Por favor, use um nome apropriado (sem ofensas).', 'erro');
      ui.el.nome.focus();
      return;
    }
    ...
    try {
      ...
      if (dados.arquivo) {
        ...
        const { base64, mime } = await FeedbackFoto.converterParaWebp(dados.arquivo, {
          maxLado: 1280,
          qualidade: 0.8,
          tamanhoMaxMB: 2,
        });
        ...
        photo_public_url =
          r.photo_public_url ||
          (r.photo_id ? `https://drive.google.com/uc?export=view&id=${r.photo_id}` : '');
      }

      // pega do hidden #rating OU do radio selecionado name="rating_star"
      const ratingHidden = Number(document.querySelector('#rating')?.value || 0);
      const ratingRadio = Number(
        document.querySelector('input[name="rating_star"]:checked')?.value || 0
      );
      const rating = ratingHidden && ratingHidden > 0 ? ratingHidden : ratingRadio;
      ...
      if (!ret.success) throw new Error(ret.message || 'Erro ao salvar feedback.');
