...
(function (w) {
  ...
  // URL real do Apps Script (produção)
  const GAS_EXEC =
    'https://script.google.com/macros/s/AKfycbzzCFgGmXhIDc7xlaJa_XpacGMu3GBn7d0kg2ntRgUrpuisnV__AjF_8pJGXgG6NaMP0A/exec';

  // Em dev: usa o proxy do Vite (/gas)
  // Em build/produção: usa o Apps Script direto
  let ENDPOINT = import.meta.env.DEV ? '/gas' : GAS_EXEC;
  ...
  function withTimeout(promise, ms = DEFAULT_TIMEOUT_MS) {
    ...
    return Promise.race([
      ...
      new Promise((_, rej) => (t = setTimeout(() => rej(new Error('Timeout ao chamar API')), ms))),
    ]);
  }
  ...
  function _coerceUrl(raw) {
    ...
    if (raw && typeof raw === 'object') {
      ...
      if (typeof raw.id === 'string') return `https://drive.google.com/uc?export=view&id=${raw.id}`;
